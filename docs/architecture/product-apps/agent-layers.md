...
> **专利业务**以 [agent-business-mode](./agent-business-mode.md) 为准（`/agent`=我的案子）；专家台见 [agent-patent-shell](./agent-patent-shell.md)（`/agent/catalog`）。
...
> **冷启动（双产品路径）**：见 §3——**通用沙盒**=L1(Solo)·`/agent/sandbox`；**专利业务**=我的案子·`/agent`（[agent-business-mode](./agent-business-mode.md)）。Catalog 已降级。  
...
- 专利业务冷启动以 [agent-business-mode](./agent-business-mode.md) 为准；专家台 [agent-patent-shell](./agent-patent-shell.md)；Pack：[../domain-packs/](../domain-packs/)。
...
专利业务：`/agent`→我的案子（business-mode）——与沙盒**不互斥**。
```
...
|----|------|
| 本稿 | 能力叠层 + **通用沙盒**入口 |
| [agent-business-mode](./agent-business-mode.md) | **专利业务**冷启动（我的案子） |
| [agent-patent-shell](./agent-patent-shell.md) | 专家工作台 `/agent/catalog` |
...
1. **已有**：业务模式规格 + 专家台双文件 mock；HITL/Command 形状在。  
2. **双路径**：沙盒 L1 · `/agent/sandbox` ∥ 业务我的案子 · `/agent`。  
...
| 做 | 不做 |
|----|------|
| 沙盒=`/agent/sandbox`→L1；业务=`/agent`→我的案子 | 仍把 Catalog 当冷启动 |
...
- 规格：架构设计  
- 壳：Agent应用助手 · 业务跟 [agent-business-mode](./agent-business-mode.md)；专家台跟 shell；沙盒跟 L1  
