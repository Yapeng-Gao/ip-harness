...
| L1/L2/L3 入口钉 | 并列入口保留；专利壳 Catalog 默认 | Solo/Team/Domain **叠名不推翻** | 已对齐（文档） |
...
2. 假设验证清单 P0 项有结果（至少：沙箱启停限额、pause/resume 或等价、dsh headless 锁版本、期限表专家核对计划）。  
3. 总控确认：Catalog 子集过渡策略与 16 席目标态不冲突。
