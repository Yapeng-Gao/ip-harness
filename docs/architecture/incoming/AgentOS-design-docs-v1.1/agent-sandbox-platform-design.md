...
| 17 | F1–F9 流程完整性（预审/优先审查/PCT 进入国家阶段/复审路径未覆盖） | 本方梳理 | 待专家核对 | 专利代理师评审流程图，缺口补进 pack v0.4 | P1 |
| 18 | 25 张 mermaid 图在目标渲染器（客户/团队所用工具）全部正常显示 | — | 待验证 | 评审前全量渲染走查（重点：stateDiagram 与含 `&lt;` 标签） | P1 |
| 19 | AionUi Team Mode 交互可用于组队中心/团队视图线框迭代 | 官方 README/演示 | 待验证 | 实装 AionUi 走一遍 Team Mode，输出线框 diff 清单 | P2 |
| 20 | aionrs 引擎可经 Driver 挂载（headless/可编程接入） | 官方文档 | 待验证 | 零配置引擎跑通非交互任务（§16 补充调研） | P2 |
| 21 | AgentScope ReActAgent 可作 L0 引擎（Python 异步/HITL steering/Runner 部署） | arXiv 论文+官方文档 | 待验证 | 与 dsh 同挂 Driver 跑同一 golden set A/B | **P0** |
| 22 | AgentScope Runtime FileSystem 沙箱满足一任务一沙箱语义 | 官方 Runtime 文档 | 待验证 | lease/destroy 生命周期 + HITL 语义映射（§16 补充调研 2） | P1 |
...
## 16. 选型决策记录：L0 引擎层「自研 vs dsh 二开」

**结论：混合路线——dsh 起步用而不深改，L1/L2 自研，Driver 边界锁死切换成本。**

### 决策依据

- dsh 恰好覆盖所需（headless / Python SDK / MCP / 子代理原语 / 模型可插拔），且恰好不需其提供 validator 与编排（设计本就将 validator 置于引擎外部、编排权集中于编排器）；
- 自研 agent loop 需 3-6 个月才能生产化，且该层不产生护城河（护城河在领域资产与回流语料）；
- 风险：dsh 为 v0.1 dev preview，接口有破坏性变更，路线受 DeepSeek 控制。

### 策略

1. **用而少改**：SDK/headless 调用为主，定制走 Cordis 插件点，不深度 fork（避免合并冲突）；必要时 vendor 单个稳定模块；
2. **切换触发器**（任一满足即启动自研 L0 或换引擎，切换成本 = 换镜像 + 改 Driver 命令行，约一周）：
   - dsh v1.0 仍不稳定，或破坏性变更撞到 ≥2 次；
   - 需求分叉：loop 内嵌 validator / 细粒度 token 计量 / 会话级租户隔离；
   - DeepSeek 路线转向；
3. **双引擎对冲**：codex app-server 与 dsh 并存于 Driver 后，A/B 跑同一 golden set。

### 状态

2026-09-21 决策 · 待验证项：假设清单 #7（dsh headless/SDK 实测）· 复评时点：dsh v1.0 发布或触发器任一命中。

### 补充调研：AionUi（2026-09-21）

iOfficeAI/AionUi（Apache-2.0, 29.5k★, v2.1.30）：Electron 桌面 + Rust 内置引擎（aionrs），统一接管 20+ CLI Agent，Team Mode（Leader/Teammate/共享看板）与我们的 Team 模式同构，远程入口（WebUI/飞书/钉钉/微信）与 Cron 开箱即用。

**结论：不作平台底座（架构假设相反——单机桌面/宿主机权限模型 vs 我们的多租户沙箱舰队），但作为三个角色采用**：
1. **Team 模式交互参考**：组队中心/团队视线的线框与其实现逐点比对（线框文档迭代依据）；
2. **第三 L0 引擎候选**：aionrs 经 Driver 挂载，与 dsh、codex 三引擎互备（对冲升级）；
3. **演示控制台**：WebUI 模式（独立 HTTP 服务）作为专利工作台前端就绪前的内部演示入口。

待验证：假设清单追加 #21（aionrs headless 可编程接入实测）。

### 补充调研 2：AgentScope（2026-09-21）

阿里达摩院开源（Apache-2.0，30.5k★，v1.0 于 2025-08 发布，arXiv 论文）。Python 原生全异步，ReActAgent（L0）、内置 HITL steering（暂停/检查/改 prompt/恢复，与 hitl.py 同构）、Runner→FastAPI 部署、OTel 可观测、MCP/A2A 原生。**AgentScope Runtime V1.0（2026-06）自带沙箱类型（GUI/Browser/FileSystem/Mobile/Cloud）与 Docker/K8s 部署**；AgentScope Java 2.0 主打分布式多租户。

**关键分歧点**：其设计哲学明确"利用模型推理而非强编排约束"（官方 README 原文）——与我们的合规强编排（签批门禁/审计链/feedback 回边）正好相反。

**修订结论**：
1. L0/L1 双候选 POC：dsh vs AgentScope ReActAgent，同挂 Driver 后 A/B（假设清单 #21）；
2. 沙箱后端三候选：OpenSandbox / CubeSandbox / **AgentScope Runtime**（尤其客户已在阿里云栈时集成阻力最小）；
3. L2 编排器 + validator + 领域包自研不变——AgentScope 明确不做的强编排正是我们的位置；
4. 风险提示：AgentScope+OpenSandbox+ACS 同为阿里系，"三候选对冲"同时是防单家绑定的手段。

待验证：#22（AgentScope Runtime FileSystem 沙箱 + HITL steering 与 hitl.py 语义映射实测）。
