...
**LLM 是通才，专家能力由四样外部资产包装而成**——类比医院：模型是医学生，SOP 是诊疗路径，工具是检验设备，validator 是主任医师签字。

| 资产 | 作用 | 落地物 |
|---|---|---|
| ① 程序性知识 | 活儿怎么干（步骤/填法/红线） | `prompts/<bot>.md` |
| ② 领域知识 | 知道什么（法条/判例/模板/公知常识） | `kb/`（断网进镜像，联网走 RAG） |
| ③ 确定性工具 | 怎么算（检索式生成/评分公式/比对器） | `tools.py` 注册 |
| ④ 裁判 | 干得对不对 | `rules/<bot>.py` validator |

设计规律：**越需要专业判断的环节，越不交给模型的"感觉"**——检索员的手艺（构造检索式）固化成代码常量，评级结论从公式出，模型只负责理解长文本和生成结构化内容。

**能力养成闭环**：冷启动（专家写 prompt + 20-30 样例进 KB）→ 上线后三层评测（过检率 / golden set / HITL 采纳率）→ 归因三层（prompt/KB/工具）→ 迭代。**HITL 驳回与人工修改是专家知识回流的主通道**——专家在系统里的角色是维护四样资产，而非重复教 AI。
...
class BotRuntime:
    ...
    async def delegate(self, task, sub_type, sandbox_policy, depth=1):
        """跨沙箱子 Run（§5.6 委托机制）：深度≤2，预算从父池扣，parent_run_id 入信封"""
        if depth > 2:
            return Handoff.escalate("委托深度超限")
        sub_spec = REGISTRY[sub_type]
        sub_lease = await self.driver.lease(self.flow_id, sub_spec.image,
                                            sandbox_policy, parent=self.lease)
        result = await BotRuntime(self.driver, self.registry, self.prompts)
        result = await result.run(sub_spec, task, self.flow_id, budget_pool=self.budget)
        await self.driver.release(sub_lease)
        return result
...
```yaml
# bots/project_review.yaml —— LLM 做区别特征分析+证据采集，公式注册为工具不可篡改
...
> 交叉引用：运行时实现的语义规则（自修复上限/escalate/熔断/回流转）以《patent-domain-pack-design.md》§2 状态机、§5.4 Loop 索引、§5.5 流程定义图为准；本文档代码是其可执行表达。
...
**A 档 · 在线检索型**（商业库 API 接入，本地不落全文）

| Bot | 数据类型 | 推荐来源 | 商业库 |
|---|---|---|---|
| 查新检索员 | 中外专利全文+法律状态 | API，本地仅缓存 | incoPat / 智慧芽 / Orbit；免费兜底 Espacenet+Google Patents |
| 行业分析师 | 专利族+引文+市场数据 | API+RAG | incoPat/智慧芽 + 万得/企查查 |
| 竞争情报员 | 竞对新公开监控流 | 预警推送 API | 智慧芽/incoPat 监控功能 |
| OA 答复代理 | 审查指南+复审无效决定+对比文件 | 内置+RAG+在线 | 复审决定库（官方公开）+商业库 |
| 价值评估师 | 引用/同族/许可历史 | API | 智慧芽/incoPat |
| 转化顾问 | 许可交易对标 | API/抓取 | ktMINE/RoyaltyStat（贵，后置）+技术交易所公开挂牌 |

**B 档 · 内置 KB 型**（打包进断网镜像，发布流水线更新）

| Bot | 数据 | 体量 | 来源 |
|---|---|---|---|
| 撰写代理人 | 法条库+审查指南（合规卖点） | <2GB | 公开法规 |
| 创新教练 | TRIZ 40 原理+领域综述 | <500MB | 内置（商用注意知网/万方授权） |
| 专利挖掘师 | 可专利性驳回判例库 | ~1GB embeddings | 官方复审决定离线加工 |
| 交底书工程师/附图工程师/布局策略师 | 模板与样例 | <1GB | 内部资产 |

**C 档 · 纯代码数据型**（无 LLM 数据需求）：流程管家（期限表=代码常量，假设清单 #17 专家核对）、立项评审（评分参数=配置）、年费管家（内部台账 DB）。

### 7.2 数据治理红线

1. 交底/申请文件按租户加密隔离，任何商业 API 调用不得带出租户数据；
2. 商业库全文缓存条款逐家确认（多数账号协议禁止全文落库，仅允许临时缓存）；
3. 断网 bot 的 KB 更新走镜像发布流水线，全程不与外网直连。

---

## 8. 流程瘦身与最小可行链路

**原则：流转能力取决于状态机 + handoff 信封，不取决于节点数量。** 两个瘦身手法：

1. **bot → 纯代码 service**：流程管家、立项评分本质是"代码做裁判"——降级为 service（无沙箱、无模型费），节点消失环节保留；
2. **bot 合并**：撰写代理人吸收附图工程师（一沙箱双 validator 串行）；交底+挖掘合并。

### 8.1 F5-lite（3 bot + 2 service，环边全保留）

```yaml
# flows/f5-lite.yaml —— 状态机与 F5 全量版完全一致
steps:
  - {bot: 交底挖掘师,    v: 六段+三段式,   hitl: HITL③}      # 交底书工程师+挖掘师合并
  - {bot: 查新检索员,    v: 覆盖度,       hitl: HITL④}
  - {bot: 撰写附图代理,  v: 四类+一致性,  hitl: HITL⑤}      # 撰写+附图合并,双validator
  - {service: 递交服务,  v: 递交清单}                        # 纯代码,无沙箱
loop_edges: [hitl_reject, self_repair, feedback, escalate]     # 与全量版相同
```

- **能跑**：编排器按带环 DAG 执行，3 个 state 与 5 个 state 无差别；
- **不能省**：HITL③④⑤（§13.1 合规）、validator 裁判、超范围红线 blocker；
- **省掉的代价**：去掉查新 → HITL④ 变盲签（执业风险）；去掉 feedback 边 → 能跑但返工靠人肉；
- **成本**：3 沙箱/任务 vs 全量 5 沙箱/任务，省约 40% 沙箱成本 → 小客户定价档。

### 8.2 瘦身决策表

| 客户档位 | 方案 | bot 数 | 保留的质量环节 |
|---|---|---|---|
| 机构版（全量） | F5 全量 5 bot | 5 | 全部 |
| 小所版 | F5-lite | 3 | HITL+validator+红线全保留 |
| 试点版 | 单 bot 直跑（仅查新+人工撰写） | 1 | 仅查新环节，无流程 |
