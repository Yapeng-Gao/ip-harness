...
| ID | 项 | 端口 | Owner | 规格 | 验收 |
|----|----|------|-------|------|------|
| B1 | **search 检索样机** | **5182** | 检索服务助手 | `architecture/search/`（`623df3d`） | 三模式 + 结果过滤同族 + 详情 + 工作篮 + Agent JSON；横幅诚实；架构评审 Pass + 总控验收 |
...
| D5 | **附图生成·编辑** | `apps/figure` 或 doc-harness 模式 | 5187 或 5178 扩 | 上下文 → 生成草图 → 编辑标注 → 版本资产 → 挂文档章 | `architecture/figure/` |
...
| 波次 | 内容 | 出口 |
|------|------|------|
| **W0（当前）** | B1 search 规格评审 + :5182 样机 | 可演示人三模式检索 |
| **W1** | C1/C2 语料运维示意 + 工作篮下游占位 | 检索故事完整 |
| **W2** | D1 FTO 规格 + 样机 | 第一条分析业务壳 |
| **W3** | D5 附图 或 D2 挖掘（二选一先做） | 与 doc-harness/search 联动 |
| **W4** | D3 激发 + D4 全景 | 演示矩阵补齐 |
| **Wx** | 五壳/并行壳 P2 打磨 | 观感 |
...
1. 收口 **B1 search**（评审 + :5182 验收）  
2. 你确认 W2 先 **FTO** 还是 **附图** 后，再建助手与规格包  
3. 本文件为权威执行表；变更需总控改版并记一笔  

**回链**：`docs/architecture/README.md` 应挂本文件（总控提交时补一行）。
