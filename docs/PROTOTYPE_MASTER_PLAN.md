...
> **口径冻结（2026-09-13 更新）**：**原型主队列 W0–W5 已收口**。额度已刷新（2026-09-18）。**方案波已定稿**；进入 **N 波优先序**（见 §9）。无总控令仍勿擅自扩壳。  
> **方案入口**：[SCHEME_WAVE.md](./SCHEME_WAVE.md)  
> **历史样机口径**：前端样机 + 内存状态机，**不做**真后台（无真库 / ES / 向量 / GPU / Spark / SSO）。  
> **分支**：`dev`
...
| D3 | **创新激发** | `apps/inspire` | 5185 | 问题/技术点 → 语义扩召卡片 → 收藏 → 送交底/挖掘 | **样机 Pass** `50e5527` · 规格 `89a5eed` |
| D4 | **产业全景图** | `apps/landscape` | 5186 | **目标产品**：行业树（系统→子系统→零部件多层级）+ 企业/业务线/地位 + 专利布局 + 竞品 + 卡脖子/围剿风险 + 前沿方向；样机先锁 **一个垂直域种子（建议汽车）** 可下钻，非全球全行业真库 | **样机 Pass** `f5d08f7` · 规格 `f0e1c28` |
...
| **W5（完成）** | **D4 全景** `f5d08f7` + **D3 激发** `50e5527` | 汽车种子可点 · **总控 Pass** · 原型主队列收口 |
| **Wx** | 五壳/并行壳 P2 打磨 | 观感 |
...
| 5181 | ai-data |
| **5182** | **search** |
| 5183–5187 | fto / mining / inspire / landscape / figure |
...
6. ~~W5 激发+全景~~ **Pass**（inspire `50e5527` · landscape `f5d08f7`）  
7. ~~原型主队列收口~~（W0–W5 样机齐）  
8. **N 波优先序（2026-09-18 · 总控自定）**  
   - ~~**N1**~~ **Go**（`6dc1675` / 复评 `6f24ea9`）
   - **N2（当前）**：样机缺口 + 并行壳 e2e L0 + e2e-hunt MVP 预备  
   - **N2**：样机缺口（跨口工作篮约定、挂章/深链、并行壳 e2e L0）  
   - **N3**：按 `search-data` / `model-training` 出落地 MVP 任务单（可再开最小实施）  
   - **N4**：case-core / 真产业图谱 **另立项**，不插队  
9. 本文件为权威执行表；变更需总控改版并记一笔  
