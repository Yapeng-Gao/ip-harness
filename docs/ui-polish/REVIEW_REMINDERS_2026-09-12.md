...
## 附录 · 与总控口径对齐（2026-09-12）

总控盘点 P0：**诚实文案 / Inbox 序 / SLA 色 / Docket 收敛**。与本文件 ID 映射：

| 总控口径 | 本文件 | 备注 |
|----------|--------|------|
| 诚实文案 | **R-P0-1**（+ R-P1-5 toast 皮） | 提醒行动与反馈诚实度 |
| Inbox 序 | **R-P1-2**（总控升 P0 则按 P0 验） | 提醒回执 / 办理分层；评估原列 P1，服从总控派工优先级 |
| SLA 色 | 已有 DS-STATUS + dash-scan-chip；若仍有色阶漂移则并入 R-P0-1/色 token 复验 | 短评未单列新 P0，以色一致验收为准 |
| Docket 收敛 | **R-P1-1**（总控升 P0 则按 P0 验） | 行内主次 CTA；+ **R-P0-2** 只读禁用可见 |

质感按总控 P0 合入后，评估短复评对照上表四条 + 本文件 R-P0-1/2 验收即可。
