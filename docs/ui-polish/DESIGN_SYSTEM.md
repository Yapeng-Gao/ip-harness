...
## 8. UX 使用习惯 · DS-UX-HABIT

全局颜色 / 布局服从 Foundations；本节约束 **使用习惯**：用户每进一页「下一步是否一眼可见、状态是否可理解、空态是否可行动、跨壳文案是否同一套跳转」。

### DS-UX-HABIT-01 · 首屏下一步

- 每个产品首页 / 办理首页须有 **单一主行动单元**（标题区 + 主 CTA，或优先办理条）。
- mid `/`：`.dash-board-top` + `.dash-next` + navy「去办理」为样板。
- 禁止首屏多个同权主按钮抢焦点。

### DS-UX-HABIT-02 · 默认展开

- 关键上下文默认应对「待你确认 / 待办」有利：Agent 待确认会话应默认展开上下文/产物，或提供常驻「上下文」chip（未读数）。
- 案详当前阶段节点默认展开（已有 `data-current` 行为应保持）。
- 禁止把完成任务所必需的面板藏到二次点击且无提示。

### DS-UX-HABIT-03 · 禁用理由可见

- 同 **DS-DISABLED-01**：闸门 / Persona / 表单未就绪时，主 CTA 旁须内联可见原因（`.agent-confirm-reason`）。
- 禁用不得仅靠透明度或 tooltip。

### DS-UX-HABIT-04 · tip / banner 上限

- 同一视口 **最多一条 sticky tip**；info 与 blocker 视觉分级（blocker 不得落在折线以下）。
- Demo 诚实横幅（`.shell-banner-demo`）可另计，但不得与 blocker tip 叠成墙。

### DS-UX-HABIT-05 · 空态须带动作

- `.ui-empty` 须含：原因一句 + **至少一个下一步动作**（链或按钮）。
- 空列 / 空看板不可只写「暂无」无出口；可折叠空列或提供「显示空阶段」开关（见深评 P1-A）。

### DS-UX-HABIT-06 · 跨壳文案与跳转

- 同入口 **单一跳转模式**（DS-FORBID F3）：AppSurfaceLinks / 顶 pill 行为一致。
- 跨壳文案统一：「进入办理」「去确认」「打开工作台」等动词对齐，禁止同义多套（如同时「办理台」「工作台」指不同无深链）。
- 产品 chrome **禁止 raw localhost URL**（F8）；用标签按钮。
