...
| 序 | 内容 | Owner | 状态 |
|----|------|------|------|
| **P1** | 业务深度扫：五壳+并行壳关键路径逻辑/空态/闸门/诚实文案；出缺口清单 + 必改 P0 | 业务深度审计 | **进行中** |
| **P2** | 架构补缺：跨壳工作篮样机策略 A 落地规格核对；search 旗标接线边界短文；case-core **仍 N4 不开工** | 架构设计 → 架构评审 | **进行中** |
| **P3** | UI/UX：N1 后残余 P2/习惯债 + 全壳抽检；有验收标准再交质感 | UI评估 → UI质感 | **进行中** |
| **P4** | 检索表现：`apps/search` 旗标接 `:5190`（可回退 mock）；不改 APP_PORTS | 检索服务 + 检索落地 | 排队跟 P2 边界 |
| **P5** | 工作篮样机策略 A：共享种子 ID + 诚实 toast；search→FTO/mining 深链可演示 | 检索服务 · FTO · 挖掘 | 跟 P1/P2 |
