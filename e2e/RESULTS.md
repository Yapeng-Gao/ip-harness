...
**本轮跑测时间：** 2026-09-21 10:10（UTC+8）· 修 L0-AG-01 地标后复跑 agent  
...
**Command（本轮）：** `npx playwright test --project=agent`（及 `-g 'S0 Home'` 可选复验）  
**webServer：** 无（agent:5175 已齐口）  
**Browser：** Chromium（headless）  
**本轮合计：** **L0-AG-01 PASS**（656ms）；agent-stepwise **S0 PASS**（1.9s，轻修同口径）  
**失败：** 无（绿盘唯一失败已消）

## 最新跑测（2026-09-21 10:08 UTC+8 · 周一）

**巡检命令：** `curl -s --max-time 2` 检查旧壳、api-mock、并行壳。  
**结论：** 关键门禁齐（5173–5177 + 5180/health 均 200 / `ok===true`），故跑非并行 project；并行壳 5182–5187 全部 DOWN，记「未跑」（不挡主绿盘，但禁止假绿）。  
**过滤原因：** 全量含 search/fto/mining/inspire/landscape/figure 会在端口 DOWN 时 Fail；故用 `--project` 排除并行六壳，只覆盖既有主门禁 15 条（原 21 − 并行 6）。未跑 `agent-stepwise`。  
**与上次完整回归相比：** 2026-09-18 为 12/12 UP、21 passed；本轮主 6/6 UP + 并行 0/6、主测 14/15 绿。  
**本轮纪律：** 未改 `apps/`；未扩 L2/Hunt；禁止假绿。
...
| agent | 5175 | `http://localhost:5175/agent` | 200 | PASS（地标已随 UI 更新） |
...
**门禁：** L0-AG-01 地标修复后 agent project 退出码 **0**。并行 DOWN 仍记「未跑」（未本轮重跑全量）。
...
## L0（零 click）· 五壳 + api（本轮 2026-09-21）

| ID | 壳 | 结果 | 备注 |
|----|----|------|------|
| L0-MID-01 | mid:5173 | PASS | heading「资产与任务」可见 · 1.4s |
| L0-WB-01 | workbench:5174 | PASS | heading「业务工作台」 · 991ms |
| L0-AG-01 | agent:5175 | PASS | 地标已随 UI 更新：`getByRole('heading', { name: /专利专家 Catalog\|办理\|Catalog/ })` · 656ms · 零 click |
| L0-OPS-01 | ops:5176 | PASS | 运维标题可见 · 952ms |
| L0-IAM-01 | iam:5177 | PASS | heading「IP Harness」 · 920ms |
...
| ID | 路径 | 结果 | 备注 |
|----|------|------|------|
| L1-01 | iam:5177 → mid「资产与任务」 | PASS | 1.5s |
| L1-02 | sess-oa-1 HITL CTA 可见（不点） | PASS | 1.5s |
| L1-03 | `/cases/c1?tab=audit`「最近领域命令」 | PASS | 1.6s |
| L1-04 | `localhost:5176/config#alerts` | PASS | 1.2s |
| L1-05 | `/workbench` + `/workbench/research` | PASS | 2.1s |
| L1-06 | `/agent/agents` 9 heading | PASS | 1.6s |
| L1-07 | sess-disclosure-1 启动 → HITL | PASS | 10.1s |
...
| L1-API-UP-01 | **5180=起**；enterWorkspace → mid | PASS | 只验读：`/v1/cases` + 地标 · 1.6s；无写/dispatch |
| L1-API-DOWN-01 | enterWorkspace → mid 后 **page.route 模拟不可达，进程仍起** | PASS | 模拟 5180 不可达 → fallback 壳可用 · 1.9s；**非**关开关 |
...
- `/agent` 现主标题 h1「**专利专家 Catalog**」（`PatentCatalogPage`）；旧「要办哪件事」/发送|启动/办理目标 已不存在。
- `e2e/l0-smoke.spec.ts` L0-AG-01 → `getByRole('heading', { name: /专利专家 Catalog|办理|Catalog/ })`。
- `e2e/agent-stepwise-s0-s2.spec.ts` S0 同步轻修：去掉 compose/办理目标/home-send/case-bind；改为 Catalog h1 + `patent-catalog-page` + 侧栏「办理导航」。
- **未改** `apps/`。
...
3. Hunt 不进本表；agent-stepwise 本轮仅复验 S0（PASS），S1+ 未跑。
4. 本轮并行壳未起 → 六条 L0 并行仍记「未跑」；未重跑全量主门禁 15 条（10:08 其余 14 已 PASS）。
5. 绿盘唯一失败 L0-AG-01 已消；全量齐口回归仍待并行壳 UP 后补跑。
