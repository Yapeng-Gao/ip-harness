...
import type { AgentRunStatus, HandoffStatus, HitlGateId } from '../../../types'
...
export function workbenchHref(
  ...
  if (workbenchPath.startsWith('/inventor')) {
    return caseId ? `${workbenchPath}?case=${caseId}` : workbenchPath
  }
  return `${workbenchPath}/${caseId}`
}
