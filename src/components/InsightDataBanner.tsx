...
import { COMMERCIAL_PROVIDERS, DATA_SOURCE_MODE_LABELS } from '../data/dataStrategy'

export function InsightDataBanner({ compact }: { compact?: boolean }) {
  ...
  if (compact) {
    return (
      <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs">
        ...
        <Link to="/settings/data" className="ml-auto text-slate-700 hover:text-slate-900">
          数据策略
        </Link>
      </div>
    )
    ...
    <div className="mb-5 flat-card p-3">
      ...
      {dataSourceMode === 'commercial_api' ? (
        ...
        <p className="rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-700">
          ...
          API 示意缓存。详见{' '}
          <Link to="/settings/data" className="underline">
            数据策略
          </Link>
          。
        </p>
...
export { COMMERCIAL_PROVIDERS as INSIGHT_DATA_SOURCES }
