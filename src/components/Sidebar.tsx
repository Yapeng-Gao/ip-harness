...
export function Sidebar() {
  ...
  const otherNav: NavItem[] =
    workspace.kind === 'agency'
      ? [
          { to: '/inventor', label: '交底门户', icon: UserCircle },
          { to: '/settings', label: '设置', icon: Settings },
        ]
      : [
          { to: '/agencies', label: '代理所市场', icon: Store },
          { to: '/inventor', label: '交底门户', icon: UserCircle },
          { to: '/settings', label: '设置', icon: Settings },
        ]
      ...
      <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto p-3" aria-label="作业中台导航">
        {inventorMode ? (
          <>
            ...
            <AppNavLink to="/inventor" className={({ isActive }) => linkClass(isActive, true)}>
              <UserCircle className="h-4 w-4" aria-hidden />
              交底门户
            </AppNavLink>
            <AppNavLink to="/cases" className={({ isActive }) => linkClass(isActive)}>
              <FolderOpen className="h-4 w-4" aria-hidden />
              返回案件库
            </AppNavLink>
            <AppNavLink to="/settings" className={({ isActive }) => linkClass(isActive)}>
            ...
            {workbenchNav
              ...
              .map((item) => (
                <AppNavLink
                  ...
                  to={item.to}
                  className={({ isActive }) =>
                    linkClass(isActive || loc.pathname.startsWith(item.to))
                  }
                >
                  <item.icon className="h-4 w-4" aria-hidden />
                  {item.label}
