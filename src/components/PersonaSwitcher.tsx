...
export function PersonaSwitcher({ variant = 'sidebar', className = '' }: Props) {
  ...
  const trigger =
    variant === 'topbar' ? (
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="btn-press focus-ring flex w-full items-center gap-2 rounded-[var(--radius-md)] border border-slate-200/90 bg-[color-mix(in_srgb,var(--color-accent-soft)_80%,white)] px-2.5 py-2 text-left hover:border-[color-mix(in_srgb,var(--color-accent)_35%,transparent)] hover:bg-[var(--color-accent-soft)]"
        aria-expanded={open}
        aria-haspopup="listbox"
        aria-label="切换 Persona"
      >
        <UserCog className="h-3.5 w-3.5 text-[var(--color-accent)]" aria-hidden />
        <div className="min-w-0 flex-1">
          <div className="truncate text-xs font-medium text-slate-900">
            {PERSONA_LABELS[persona]}
          </div>
          <div className="truncate text-[10px] text-slate-500">演示 · 可执法</div>
        </div>
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      {open && (
        <div
          className={`menu-enter absolute z-40 mt-1 overflow-hidden rounded-[var(--radius-md)] border border-slate-200 bg-white elevated ${
        ...
        >
          {PERSONAS.map((p) => (
            <button
              ...
              onClick={() => pick(p.id)}
              className={`btn-press focus-ring flex w-full flex-col gap-0.5 px-3 py-2.5 text-left hover:bg-[var(--color-accent-soft)] ${
                p.id === persona ? 'bg-[var(--color-accent-soft)]' : ''
              }`}
            >
