...
import { useAgents, type HitlSessionAction } from '../../context/AgentContext'
...
import type { HitlGateId } from '../../types'
import {
  evaluateGuardrails,
  firstGuardrailMessage,
  hasVerifiableResearchHitsFromSession,
} from '../../domain/guardrails'
import { COMMAND_LABELS } from '../../domain/commands'
import { setLastAgentSessionId } from '../../utils/lastVisited'
import { agentDisplayLabel } from '../../hooks/useAgentDisplayLabel'
import { SessionConfirmBar } from '../../components/agent/session/SessionConfirmBar'
import { SessionTimeline } from '../../components/agent/session/SessionTimeline'
import { SessionContextPanel } from '../../components/agent/session/SessionContextPanel'
import { SessionComposer } from '../../components/agent/session/SessionComposer'
import {
  SessionWorkspaceHeader,
  type BannerKind,
} from '../../components/agent/session/SessionWorkspaceHeader'
import {
  DEMO_AGENTS,
  gateToAction,
  sortGatesForRole,
          ...
          <div className="toast-enter pointer-events-none fixed bottom-6 right-6 z-50 max-w-sm">
            {domainSynced && (sess.caseId || toastWbOverride) ? (
              <div className="pointer-events-auto flex flex-wrap items-center gap-2 border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-950 shadow-lg">
                ...
                {toastCaseId && (
                <AppLink
                  to={`/cases/${toastCaseId}?from=agent`}
