...
对照样机：左文档树（含 **CaseSwitcher**）· 中 **TipTap 纸面文档** · 右 **批注 | Agent**（Tab；默认批注）+ HITL Confirm（在 Agent tab）。  
...
| 能力 | 现状 |
|------|------|
| 多案切换 | 顶栏 + 左树 `CaseSwitcher` · ≥3 案 |
| TipTap 纸面 | 工具栏 · 页感 · annotation Mark |
| 批注 | 侧栏内联新建（**无** `window.prompt`）· Mark↔列表联动 · 已解决折叠/过滤 · orphan 提示 |
| Agent mock | 按 chapter key 模板改写 · 试运行/正式 · Confirm · 逐段 diff |
| 批注保真 | 采纳时按 quote 重挂 open 批注；失败 → orphan |
| SKU/章闸 | 附图说明 `locked` → 只读 + slate CTA；`authorized:false` 类型已预留 |
| 未保存离开 | **自动 saveDraft** + 顶栏「已自动保存」 |
| Cmd/Ctrl+S | 保存草稿 |
...
2. 点 Mark ↔ 侧栏高亮（accent-soft）；已解决可折叠/过滤。
3. 空选区禁用；无批注 / 全解决 empty state：`surface-50` + caption + navy CTA。

## Harness loop（样机）

1. **试运行**：mock 建议预览 + 逐段对照，**不**落库、**不**进 Confirm 写入  
2. **正式建议**：`DocProposal` → ConfirmBar  
3. **确认**：mock dispatch `submitClaims` → 批注按 quote 重挂 → 新 revision（actor=agent）→ `setContent`  
4. **拒绝**：丢弃建议；无 command、无 revision  
5. **保存草稿** / 自动保存：`saveDraft` revision（actor=user）
