...
import { HONESTY_BANNER, SEARCH_DEEPLINK, STEPS } from '../state/types'
import { useFtoStore } from '../state/store'
import { Chip } from '../components/ui'

const NAV = [
  { to: '/features', label: '① 产品特征', icon: ListTree },
  { to: '/hits', label: '② 检索命中', icon: Search },
  { to: '/matrix', label: '③ 对比矩阵', icon: Grid3X3 },
  { to: '/risk', label: '④ 风险等级', icon: Shield },
  { to: '/report', label: '⑤ 报告 Confirm', icon: FileCheck2 },
] as const
...
export function FtoShell() {
  const loc = useLocation()
  const { hits, matrix, features, report } = useFtoStore()
  ...
  return (
    <div className="app-shell-bg flex min-h-screen flex-col text-slate-900">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2 text-xs text-slate-600">
        <span>FTO 分析平面 · 消费 Search Hit · 不改 APP_PORTS</span>
      ...
      <div className="border-b border-slate-200 bg-white px-4 py-2">
        <ol className="mx-auto flex max-w-5xl flex-wrap items-center gap-1.5">
          {STEPS.map((s, i) => {
            ...
            const warn =
              (s.path === '/matrix' && hits.length === 0) ||
              (s.path === '/report' &&
                (features.length === 0 || matrix.cells.length === 0))
            return (
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="shell-aside hidden w-56 shrink-0 lg:block">
          <div className="px-4 pt-5 pb-2">
            <p className="shell-page-kicker">apps/fto</p>
            <p className="mt-1 text-[15px] font-semibold tracking-tight text-slate-900">
              FTO 自由实施
            </p>
          </div>
          <nav aria-label="FTO 步骤" className="flex flex-col gap-0.5 px-3 py-2">
            {NAV.map((item) => {
